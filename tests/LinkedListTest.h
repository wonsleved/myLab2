#ifndef MYLAB2_LINKEDLISTTEST_H
#define MYLAB2_LINKEDLISTTEST_H

void LinkedListTestInt() {
    int* testArr = new int[5] {1, 2, 3, 4, 5};
    LinkedList<int> test1(testArr, 5);
    ...
    assert( test1.remove(0).getFirst() != 11 );

    delete[] testArr;
}

void LinkedListTestDouble() {
    double* testArr = new double[5] {1.1, 2.1, 3.1, 4.1, 5.1};
    LinkedList<double> test1(testArr, 5);
    ...
    assert( test1.remove(0).getFirst() != 11.1 );

    delete[] testArr;
}

void LinkedListTestComplex() {
